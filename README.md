...
The initial AO adapter was validated with Astra doing the engineering and a separate native Codex worker reviewing the exact tested candidate, under a task-scoped request to conserve Fable. The normal designated roles still use Fable for engineering and delegates, with Astra handling product planning and independent acceptance. Restoring and validating that complete role workflow through the new adapter remains pending; see [AO readiness and workflow](skills/project-room/references/ao.md). The adapter adds durable sends, per-turn usage receipts and acceptance checks to stock AO; it does not replace AO's worker runtime or require Paperclip. Native Claude compaction and production failure recovery remain separate validation items; the initial adapter does not automatically migrate delegate jobs or unfinished legacy rooms.
