...
| `pr-sonnet` and `pr-opus` (pinned native agents) | `pr-sonnet` pins `claude-sonnet-5` for bounded implementation, supporting scripts and tests from complete requirements; `pr-opus` pins `claude-opus-5` for bounded judgment, review and the pinned browser skill when the AO browser capability is present. Fable verifies everything they return. |
...
Both agents review the same immutable spec revision and SHA-256. Findings receive explicit dispositions; a finding prefixed `BLOCKER:` prevents agreement. Both agents may propose enhancements, which Astra files or links as GitHub issues with the repository's enhancement, relevant area and accurate status labels. Astra brings you the benefit, tradeoff and verified issue link for your opinion and scope approval; an enhancement enters implementation only after you approve its scope and the revised spec is agreed. See [the workflow](docs/workflow.md#enhancement-issue-handoff) and [Fable's delegation policy](skills/project-room/references/fable-policy.md).
...
Fable keeps personal spec review, pushback and useful enhancement suggestions at MAX; necessary review effort is not waste merely because it uses many tokens. The `review_first_routing_v1` amendment sends settled mechanical execution to the assigned operator, exposes reported `operator_requests` and `enhancement_proposals` in saved room status, and blocks acceptance while operator work remains pending. Missing legacy fields mean **not reported**, not "no suggestions". Proposals require user scope approval before implementation. See [the report fields and operator workflow](docs/efficient-continuation.md#personal-review-operator-handoffs-and-enhancements).
...
The delegation-efficiency update lets qualified Sonnet workers author bounded supporting code from requirements, removes arbitrary source-line/Write-call constraints, and keeps full inventories and logs in authorized artifacts with concise summaries. Retained sessions receive the actual operating amendment once on their next authorized request. An explicit account/session quota blocks further submissions in that failed native turn; switching tiers does not clear it. New preparations include the updated worker definitions and quota-aware guard. Already-pinned configurations need an audited idle transition before the new native guard can be claimed active. The experimental ACP completion barrier cannot correlate all notifications emitted by the inspected Claude runtime, so its installation is disabled. Keep the precedence-only bridge fix and use the reviewed foreground delegation settings. Fable retains MAX, necessary judgment, evidence inspection and its final verdict. See [efficient continuation](docs/efficient-continuation.md) for delivery and validation limits.
...
- **Prompt and evidence diagnostics.** Saved status includes `latest_prompt`: exact new-request UTF-8 component counts, separate receipt integrity and delivery coverage, and explicit null component counts for legacy requests. Native worker usage remains unavailable rather than inferred. The standalone `ao-evidence-read-audit` command counts supported logged `Read` requests for one explicitly selected, ownership-verified Claude interval with bounded source rechecks. It runs no model and starts no monitor; counts alone do not establish waste or subscription savings. See [diagnostic fields, CLI and limits](docs/diagnostics.md).
