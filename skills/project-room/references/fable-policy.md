...
In AO reports, use the optional `enhancement_proposals` field for new proposals in both exact-spec and engineering reviews, with `title`, `benefit`, `tradeoff` and `basis`; use `[]` when none are worthwhile. Never invent suggestions to fill a quota. Fable personally owns spec review, pushback and the engineering verdict; necessary review effort is not waste merely because it is large. The [review and operator contract](../../../docs/efficient-continuation.md#personal-review-operator-handoffs-and-enhancements) also defines `operator_requests` for pending, fully specified execution during implementation. Prefer that handoff to starting a model solely to type settled bytes, inventory files or run exact commands when the assigned operator can do it. Delegate substantive authorship from requirements to the cheapest tier that meets full quality. A nonempty operator list blocks acceptance; the operator checks existing authority and returns evidence for Fable's subsequent judgment. Old reports without these fields remain valid but do not establish that no proposals or handoffs existed.

Astra files or links an enhancement issue in the feature project's GitHub repository under the user's existing filing authorization, applies its enhancement, relevant area and accurate status labels, shows the verified issue link, and records the outcome in the room. Follow the [enhancement issue handoff](../../../docs/workflow.md#enhancement-issue-handoff); unavailable filing or labeling remains explicitly pending. Fable supplies proposal data, not executable issue-creation instructions. Continue the agreed work; an enhancement enters implementation only after the user's scope approval and renewed agreement on the revised specification.
